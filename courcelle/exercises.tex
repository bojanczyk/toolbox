...
the decomposition tree has the same shape as the tree, root bag contains the root and every other
...
If there were a cycle $C = \{v_1, \ldots, v_n, v_1\}$ then also $\tw(C) = 1$, so it is enough to show
...
paths intersect. Therefore if we follow $\rho_1, \ldots, \rho_n$ we get a nontrivial loop of bags, contradiction with
