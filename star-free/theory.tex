...
\begin{enumerate}
	...
	\end{align*}
	The neutral element is  identity relation, which is happens to be a function.
\end{enumerate}
...
A prefix is a special case of an infix. Therefore, every prefix class is contained in some infix class. One could image that it is possible to grow in the prefix ordering while staying in the same infix class. As the following lemma shows, this is not possible in a finite monoid, and therefore all prefix classes contained in the same infix class must be incomparable in the prefix ordering.
\begin{lemma}\label{lem:infix-prefix}
	Consider a finite monoid $M$, and let $m,n \in M$ be such that $m$ and $n$ are infix equivalent, and $m$ is a prefix of $n$. The $m$ and $n$ are prefix equivalent. 
\end{lemma}
\begin{proof}
	By the assumption that $m$ is a prefix of $n$, one can obtain $n$ from $m$ by multiplying to the right by some $x \in M$. By the assumption that $n$ is an infix of $m$, one can obtain $m$ from $n$ by multiplying to the left by some $y \in M$ and multiplying to the right by some $z \in M$. By iterating this process in a loop, we see that 
	\begin{align*}
	...
	\begin{align*}
	n = z^im(xy)^i x = z^jm(xy)^i \quad  \text{ is a prefix of } \quad  z^jm(xy)^j  = m.
	\end{align*}
...
\label{sec:star-free}
To illustrate the power of monoid, we will use them to describe star-free languages. These are defined by using regular expressions which cannot use the Kleene star operation, but which are allowed to use complementation.
...
h: \Sigma^* \to M.
\end{align*}
We will show that every language recognised by this homomorphism is star-free. The interseting case is when the accepting set is a single monoid element $m \in M$, in which case the  recognised language is
\begin{align*}
...
\begin{lemma}\label{lem:star-free-induction}
	For every $m \in M$, there language $L_m$ is a star-free.
\end{lemma}
...
\begin{proof}
	...
	\begin{itemize}
		\item[(*)] is $n \in M$ is both prefix and suffix equivalent to $m$, then $n = m$.
	\end{itemize}
